...
# quick data review
msft <- dbGetQuery(nqdb, statement = "select RefDate, Symbol, Timestamp,
Price, Quantity from trds where Symbol='MSFT' limit 10")
msft
...
msft$new <- with(msft, my_fun)

my_fun(5)

apply(msp, 2, function(x) x+10)

# syntax 1
msft$newc <- msft$Price*10

# syntax 2
msft <- transform(msft, newd = Price+10)
msft

pd <- msft

apply(msft[,c('Price', 'Quantity')], 1, function(x) sum(x))
...
msft$newe <- mapply(func, msft$Price, msft$Quantity)

# dplyr
library(dplyr)

msft <- mutate(msft, newf = Price/10)
...
spread_func <- function(a,b,p) {
  bm = (a + b)/2.0
  sp = abs(p - bm)/bm
  return(sp)
}

qts_trd_spread <- qts_trd_2
qts_trd_spread$Spread <- mapply(spread_func, qts_trd_spread$AskPrice, qts_trd_spread$BidPrice, 
                                qts_trd_spread$Price)

head(qts_trd_spread, n=10)
tail(qts_trd_spread, n=10)

head(qts_trd_spread_better, n=10)
tail(qts_trd_spread_better, n=10)

qts_trd_spread_better <- qts_trd_spread
